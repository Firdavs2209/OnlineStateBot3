...
async def search_ads_handler(message: Message):
    ...
    if any(char.isalpha() for char in message.text):
        ...
        matching_ads = db.qidiruv(keyword)
        if matching_ads:
            keyboard = InlineKeyboardMarkup(row_width=1)
            for i, ad in enumerate(matching_ads[:10], start=1):
                button_text = f"Ad {i}"
                callback_data = f"show_ad_{i}"
                keyboard.add(InlineKeyboardButton(button_text, callback_data=callback_data))
            await message.reply("Here are the matching ads:", reply_markup=keyboard)
        else:
            await message.reply("No matching ads found.")
    else:
        await message.reply("Please enter a valid keyword.")
